...
proc draw_graph {name spec {do_xv 0}} {
  ...
  update
  .c postscript -file images-gen/$name.ps -width [expr {$x1+2}] -height [expr {$y1+2}]
  global DPI
  .c delete bgrect
  
  # exec convert -density ${DPI}x$DPI -antialias $name.ps $name.png
  exec convert -density $DPI -monochrome images-gen/$name.ps images-gen/$name.png
  # exec rm $name.ps
  #if {$do_xv} {
