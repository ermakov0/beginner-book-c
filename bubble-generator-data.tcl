...
#  if2 {
#    stack
#      {line /if ( expression ) statement}
#      {line {opt /else statement}}
#  }

# {or {line {toploop expr ,}} {} *}
...
set all_graphs {
  ...
  example-digit {
    line nonzero-digit {optx {toploop digit}}
  }

  id { line id }
  int { line /int }
  const-int { line const-int }
  const-float { line const-float }
  const-char { line const-char }
  const-string { line const-string }

  if {
    line /if ( expression ) statement {opt /else statement}
  }
